...
function dealPunctuation(text) {
  return String(text)
    ...
    .replace(/((?:(?<!\d)[1-9]\s*(?:\d\s*){3}[-年.\/])(?:(?:\s*\d){1,2}(?:\s*[-月.\/]))?(?:(?:\s*\d){1,2}(?:\s*[日号])?)?(?:\s*(?:\d\s*){1,2}[:时])?(?:\s*(?:\d\s*){1,2}[:分])?(?:\s*(?:\d\s*){1,2}秒?)?(?:\s*许|左右)?)/g, '{{time:$1}}')
    // 地点
    // 机构
    // 人名
    // 物品
    ...
    .replace(/((?:[\d.]\s*)+(?:人\s*民\s*币|现\s*金))/g, '{{item_name:$1}}')
    .replace(/((?:人\s*民\s*币|现\s*金)(?:\s*[\d.])+)/g, '{{item_name:$1}}')
    .replace(/(银\s*行\s*卡(?!\s*号))/g, '{{item_name:$1}}')
    .replace(/(信\s*用\s*卡(?!\s*号))/g, '{{item_name:$1}}')
    ...
    .replace(/(视听资料)/g, '{{item_name:$1}}')
    .replace(/(视频资料和图片)/g, '{{item_name:$1}}')
    .replace(/((?:到|破)案经过)/g, '{{item_name:$1}}')
    .replace(/(道路交通事故现场图)/g, '{{item_name:$1}}')
    .replace(/(现场勘查笔录)/g, '{{item_name:$1}}')
    .replace(/(当事人陈述材料)/g, '{{item_name:$1}}')
    .replace(/(测试检测结果单)/g, '{{item_name:$1}}')
    .replace(/(呼吸式酒精测试结果)/g, '{{item_name:$1}}')
    .replace(/(驾驶人信息查询结果单)/g, '{{item_name:$1}}')
    .replace(/(机动车信息查询结果单)/g, '{{item_name:$1}}')
    .replace(/(机动车被盗抢信息查询)/g, '{{item_name:$1}}')
    ...
    .replace(/(强制措施凭证)/g, '{{item_name:$1}}')
    .replace(/(补充侦查报告书)/g, '{{item_name:$1}}')
    .replace(/(扣押物品强制措施凭证)/g, '{{item_name:$1}}')
    .replace(/(犯罪嫌疑人户籍证明)/g, '{{item_name:$1}}')
    .replace(/(民事诉讼材料)/g, '{{item_name:$1}}')
    .replace(/(贷款合同)/g, '{{item_name:$1}}')
